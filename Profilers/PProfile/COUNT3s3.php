...
{ 
$Csem = $_POST['csem'];
$RESULT = $connect->query("SELECT count(*) FROM basicdetails WHERE `Approve`='1' AND Sem>='$Csem'");
$data=$RESULT->fetch_assoc();
echo "<br><h3>Students in Semister '$Csem'&nbsp:&nbsp";
echo $data['count(*)'];
echo "</h3>";
$sql = $connect->query("SELECT * FROM basicdetails WHERE `Approve`='1' AND Sem='$Csem' ORDER BY Branch");
